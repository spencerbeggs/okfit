...
const seeded = async (): Promise<{ readonly cwd: string; readonly env: Record<string, string> }> => {
...
const seededWithoutIdentity = async (): Promise<{ readonly cwd: string; readonly env: Record<string, string> }> => {
...
describe("okfit verify (e2e)", () => {
	...
	it("--dry-run exits 0, prints the would-verify line, and writes nothing", async () => {
		const { cwd, env } = await seeded();
		const before = await readProject(cwd);
	...
	it("exits 3 with the unresolved-actor message when git has no identity (V-7)", async () => {
		const { cwd, env } = await seededWithoutIdentity();
		const before = await readProject(cwd);
