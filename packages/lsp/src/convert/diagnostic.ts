...
import { toLspRange } from "./range.js";
 ...
 * position is computed from the text (via `toLspRange`) so it lands
 * correctly across a multi-line span. Without `text`, the end is the start
 * plus the range's length on the same line. A range-less diagnostic gets the
 * zero range.
 *
 * @public
 */
