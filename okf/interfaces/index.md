...
* [okfit CLI — validate, init, context, verify, sync, lint, graph, stale](cli-commands.md) - The okfit command line's subcommands, their flags, exit codes, and JSON envelopes.
* [okfit config file schema](okfit-config-schema.md) - The .config/okfit.toml schema, its discovery order, and the defaults every key falls back to when unset.
