...
* Every edit a code action or command produces is computed against the document's current editor text and sent as a versioned `documentChanges` entry, so a client refuses it once the buffer has moved on
