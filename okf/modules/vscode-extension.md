...
- `src/extension.ts` (`vscode/src/extension.ts:1-170`) -- the extension
  ...
  `TreeView`, wires `updateStatus` to the active-editor and diagnostics
  events, and serializes every client restart (a `okfit.lsp.serverPath`
...
`resolveServer` (`vscode/src/resolve-server.ts:25-41`) picks the
...
The extension host of VS Code 1.100+ runs Node in-process (Node 24.18.1 /
Electron 42.10.0 as of this writing), so `client.ts`'s `serverOptions`
launches every source with `TransportKind.stdio`: a `command` launch for
the setting and workspace sources, a `module` launch for the bundled
source, both over stdio -- no PATH Node is required for any of the three.
A `client.start()` failure is not retried automatically; see Layout above.
...
- **OKF: Validate Bundle** (`okfit.validateBundle`) -- re-requests the
  ...
  status item; the server already revalidates on watched-file and document
  changes, so this refreshes the client's view of the last published
