...
- `okfit init` scaffolds the two new directories. Existing bundles are unaffected: the types, kind, and tags are available, not required. [#114][#114]
...
- `Module` gains two optional fields: `layer` (the module's position in the repository's dependency-layering scheme) and `pins` (paths to sibling modules it is exact-version-pinned alongside). [#64][#64]
