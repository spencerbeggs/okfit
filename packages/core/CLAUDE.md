...
- `merge` is pure and never mutates; arrays replace wholesale. Core exports the `OkfitConfigFile` tag and `okfitConfigDocumentFields` (the annotated struct `@okfit/engine`'s `lib/configs/schemastore.config.ts` projects into the published JSON Schema); discovery, XDG, the layer, and the schema's `HostedSchema` identity (`okfitConfigSchemaHost`) belong to the engine, which owns `init`.
