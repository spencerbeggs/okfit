...
[types.Module.fields.layer]
description = "The dependency layer this module sits in under the repository's own layering scheme, for example L2, so a reader can query every module at one layer."

[types.Module.fields.pins]
description = "Paths to the sibling Module concepts this module is exact-version-pinned alongside, one entry per sibling."
kind = "path"
...
values.runtime = "A runtime binding or platform capability consumers invoke rather than a shape they write: worker bindings, environment contracts, platform flags."
...
guidance = "Write the steps in the order they are performed, not as rules. Name the trigger that starts the procedure and the observable state that means it succeeded."

[types.Runbook.fields.resource]
description = "A path relative to this concept file to the script, workflow, or config the procedure runs through, when one exists in this repository."
kind = "path"

[types.Glossary]
description = "A term this repository uses in its own sense, one term per concept."
guidance = "Define the term as this repository means it, not as the wider ecosystem means it, and say plainly where the two differ. A term earns a concept on a collision or a trap; one that merely names a Module belongs in that Module."

[types.Limitation]
description = "A known edge of a contract: something that does not work, and why that is acceptable."
guidance = "State the condition that triggers it and the observable symptom, not just the cause, and name the Interface or Module whose promise it bounds. If it is fixable and merely unfixed, say what the fix would take."

[types.Limitation.fields.bounds]
...
guidance = "Document the shape from the maintainer's side: what an entry contains, what is derived from it, and what breaks if an entry is wrong. Distinct from Interface, which documents a promise to consumers."
required = ["resource"]

[types.DataModel.fields.resource]
description = "A path relative to this concept file, normally escaping the bundle, to the file or directory that holds the structure."
kind = "path"
...
[tags.dx]
description = "Concerns the experience of the people and agents who author, build, and debug the code."

[tags.ci]
description = "Concerns the unattended path: what runs without a human present, and how it fails."
