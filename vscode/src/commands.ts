...
export const registerCommands = (
	...
	useCommand("okfit.validateBundle", async () => {
		...
		if (client !== undefined && supportsCommand(client, "okfit.revalidate")) {
	...
	useCommand("okfit.setStatus", async (arg?: unknown) => {
		...
		try {
			const result = await executeOnServer<ApplyWorkspaceEditResult>(client, "okfit.setStatus", [uri, picked.label]);
			if (!result.applied) {
				void vscode.window.showErrorMessage(result.failureReason ?? "okfit.setStatus failed.");
			}
		} catch (error) {
	...
	useCommand("okfit.markVerified", async (arg?: unknown) => {
		...
		if (uri === undefined) return;
		try {
			const result = await executeOnServer<ApplyWorkspaceEditResult>(client, "okfit.markVerified", [uri]);
			if (!result.applied) {
				void vscode.window.showErrorMessage(result.failureReason ?? "okfit.markVerified failed.");
			}
		} catch (error) {
			void vscode.window.showErrorMessage(error instanceof Error ? error.message : String(error));
		}
