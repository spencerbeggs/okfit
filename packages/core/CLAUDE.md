...
- `merge` is pure and never mutates; arrays replace wholesale. Core exports the `OkfitConfigFile` tag, `okfitConfigDocumentFields` (the annotated struct `@okfit/engine`'s `lib/configs/schemastore.config.ts` projects into the published JSON Schema), and `CONFIG_SCHEMA_VERSION`, that schema's `major.minor` label — core owns the number because it owns the shape; additive optional key = minor, removed/retyped key = major, and engine's `okfitConfigSchemaHost` derives from it rather than restating it; discovery, XDG, the layer, and the schema's `HostedSchema` identity (`okfitConfigSchemaHost`) belong to the engine, which owns `init`.
