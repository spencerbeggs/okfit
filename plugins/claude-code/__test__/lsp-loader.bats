#!/usr/bin/env bats
# lsp-loader.bats — covers bin/start-lsp.sh (Task 8). No sibling-repo
# analogue: unlike vitest-agent's own loader, this one never dispatches
# through a package-manager binary — it execs node_modules/.bin/okfit-lsp
...
LOADER="$PLUGIN_ROOT/bin/start-lsp.sh"
...
# PROJECT_DIR/node_modules/.bin/okfit-lsp that records its invocation to
# PROJECT_DIR/okfit-lsp.invoked and exits exit_code (default 0).
_local_bin() {
	code="${1:-0}"
	mkdir -p "$PROJECT_DIR/node_modules/.bin"
	cat >"$PROJECT_DIR/node_modules/.bin/okfit-lsp" <<EOF
#!/bin/sh
printf 'okfit-lsp %s\n' "\$*" >"$PROJECT_DIR/okfit-lsp.invoked"
printf '%s\n' "\${OKFIT_PROJECT_DIR:-}" >>"$PROJECT_DIR/okfit-lsp.invoked"
exit $code
EOF
	chmod +x "$PROJECT_DIR/node_modules/.bin/okfit-lsp"
}
...
@test "execs node_modules/.bin/okfit-lsp directly, with no package manager at all" {
	_local_bin
	_fake_bin pnpm
	run sh "$LOADER"
	[ -f "$PROJECT_DIR/okfit-lsp.invoked" ]
	[ ! -f "$FAKE_BIN/pnpm.invoked" ]
	[[ "$output" != *"Detected package manager"* ]]
}

@test "prints the pnpm install line and execs npx when okfit-lsp is absent" {
	_fake_bin npx
	printf '{"packageManager":"pnpm@9.0.0"}' >"$PROJECT_DIR/package.json"
	run sh "$LOADER"
	[[ "$output" == *"pnpm add -D @okfit/plugin"* ]]
	[ -f "$FAKE_BIN/npx.invoked" ]
	grep -q '@okfit/lsp' "$FAKE_BIN/npx.invoked"
}

@test "prints the npm install line and execs npx when okfit-lsp is absent" {
	_fake_bin npx
	run sh "$LOADER"
	[[ "$output" == *"npm install --save-dev @okfit/plugin"* ]]
	[ -f "$FAKE_BIN/npx.invoked" ]
...
@test "exports OKFIT_PROJECT_DIR before exec'ing the local bin" {
	...
	run sh "$LOADER"
	[ -f "$PROJECT_DIR/okfit-lsp.invoked" ]
	second_line="$(sed -n '2p' "$PROJECT_DIR/okfit-lsp.invoked")"
	[ "$second_line" = "$PROJECT_DIR" ]
}

@test "forwards --stdio to the local bin" {
	_local_bin
	run sh "$LOADER" --stdio
	first_line="$(sed -n '1p' "$PROJECT_DIR/okfit-lsp.invoked")"
	[ "$first_line" = "okfit-lsp --stdio" ]
}
